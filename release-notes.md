...
- Deleting a hole in Vulcan now renumbers the remaining holes
- Inserting a hole in Vulcan names it from the blast naming scheme
...
- Re-importing a Kirra Vulcan export keeps bench and sub-drill right
...
- The Vulcan export notice is an amber bar, not a text blob
- Vulcan export now creates real blast holes, ready to use

**Bug Fix**

- The Vulcan export dialog says what it does and does not carry
...
- Exported Vulcan holes are red, not green
...
- Vulcan exports arrive in Kirra's colours, labelled id and depth
...
- Vulcan holes with several intervals now survive a round trip
...
- Exported hole metadata now matches the exported geometry exactly
...
- Vulcan blasts with multiple hole intervals now import correctly
- Exported blast holes declare their metadata length correctly
...
- Vulcan no longer crashes loading an exported blast
...
- A misdetected row can no longer give a hole an impossible spacing
...
- Hole labels size themselves to the pattern, small on tight rows

**Bug Fix**

- Vulcan exports label holes at the size Kirra draws them
...
- Exported Vulcan hole labels are sized to the hole, not the page
...
- Vulcan now accepts exported blasts without needing a surface
...
- Hiding a group now excludes it from exports
...
- Vulcan now accepts exported blasts instead of failing on memory
...
- Vulcan exports now match the blast format your Vulcan writes
...
- Vulcan exports can carry a matching drill rig library
...
- Vulcan rig names now survive an import and re-export unchanged
- Imported Vulcan holes keep their real diameter and hole type
...
- Hole tracks stay readable in light and dark themes
- A hole's colour applies to its collar, not its track
...
- Dragging in 3D now follows the mouse from where you grabbed
...
- The 3D view no longer jumps when you release the mouse
...
- Moving a hole in a section keeps it on the section
...
- Editing in a section no longer throws holes across the pit
...
- Rotating a section now turns about the section itself
...
- A section now shows the holes it cuts
...
- A new section is a 2 metre slice by default
- Section slice thickness and step are remembered between sessions

**Bug Fix**

- The section view frames the full hole depth
...
- Draw a guideline and click it to set the section plane
- Reverse a section to look the other way along it
...
- Section distances are now named Look Forward and See Behind
- A section band is bounded on both sides
...
- Pan and scroll zoom work when looking along a section
...
- A pick button chooses new section points at any time

**Bug Fix**

- Section slab widths no longer make the scene vanish
...
- Draw a line in plan and it becomes the section
- Defining a section switches to 3D and turns the view
...
- Section planes can be built from a line, not an axis
- The section plane list says what each axis slices
...
- Section plane fields are compact and stay inside the dialog
...
- The section plane dialog lays out cleanly
...
- The section plane dialog no longer scrolls sideways
...
- Section planes step forward and back with Page Up and Down
- Section step size is settable, and defaults to 0.1

**Bug Fix**

- Turning on a section plane switches to 3D
...
- Flyrock model documentation corrected and expanded
...
- Removed the flyrock stem eject angle, which could not affect any result
...
- A flyrock shroud records only the settings its model used
...
- Flyrock K constant now shows published values by rock type
...
- Rock density is entered in grams per cubic centimetre
...
- Out-of-range flyrock settings are now rejected
...
- New Roth flyrock model where rock density sets the throw
- Flyrock reads detonation velocity from your charged products
...
- Flyrock inputs a model cannot use are now greyed out
- Every flyrock setting explains which way it moves the answer
- McKenzie shroud reports the fragment size reaching its edge
...
- McKenzie reports the fragment size that reaches maximum range

**Bug Fix**

- Lundborg flyrock ranges corrected, were half the published distance
- McKenzie flyrock range no longer under-predicts poorly stemmed holes
...
- Cross-shaped holes now look the same in 3D as 2D
...
- Hole shapes now keep their bearing when the plan view is rotated
...
- Pattern templates can set the colour and shape of new holes
...
- Set a hole's colour and shape from the new Additional tab
- Hole shapes are circle, triangle, square, cross or diamond
...
- Printed plans show hole shapes and colours, matching the screen
...
- Draw holes as a circle, triangle, square, cross or diamond
- Directional hole shapes point along the hole bearing
...
- The Text tool's formula palette now follows the light and dark theme
...
- Build blast group rules by clicking chips instead of typing
- Ready-made group rules for uncharged, undelayed and short holes
- Group rules offer the hole types and colours in your blast
- Formula mistakes are explained in plain words as you type

**Bug Fix**

- A mistyped single equals no longer selects every hole
- Assign Group shows how many holes a rule matches
- Group dialogs no longer show two scrollbars at once
...
- Editing a large mesh now highlights the triangle under the cursor
